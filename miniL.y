...
D: D D1 SEMICOLON {printf("declaration -> declaration declarations SEMICOLON\n"); }| { printf("declaration -> epsilon\n");};
D1: identifier COLON A INTEGER {printf("declarations -> IDENT COLON arrays INTEGER\n");};
A: ARRAY L_SQUARE_BRACKET NUMBER R_SQUARE_BRACKET OF {printf("arrays -> ARRAY L_SQUARE_BRACKET NUMBER %d R_SQUARE_BRACKET OF\n", yylval); }
...
S2: IF BE THEN S1 ES ENDIF { printf("statement2 -> IF bool_exp THEN statements else ENDIF\n"); }
  ...
  | WHILE BE BLOOP S ENLOOP   {printf("statement2 -> WHILE bool_exp BLOOP statements ENLOOP\n");}
